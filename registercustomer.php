...
if(isset($_POST['submit'])){
   ...
   if($select->rowCount() > 0){
      ...
      if($pass != $cpass){
         ...
         $insert->execute([$email, $pass, 'customer']);
	...
	if($getuid->rowCount()>0){
		while($row = $getuid -> fetch(PDO::FETCH_ASSOC)){
			$userid=$row['id'];         
         $insert = $conn->prepare("INSERT INTO customer(cid,name,address,pincode,phno,image) VALUES(?,?,?,?,?,?);");
         $insert->execute([$userid, $name, $add,$pin,$phno,$image]);
         if($insert){
