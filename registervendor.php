...
class Vendors{
   public function RegisterVendor($conn){
...
if(isset($_POST['submit'])){
   ...
   if($select->rowCount() > 0){
      ...
      if($password != $cpass){
         ...
         $insert->execute([$emailId, $password, 'vendor']);
      }
   }

	$getuid = $conn->prepare("SELECT id FROM users WHERE email = ?;");
	$getuid->execute([$emailId]);

	if($getuid->rowCount()>0){
		while($row = $getuid -> fetch(PDO::FETCH_ASSOC)){
			$userid=$row['id'];         
         $insert = $conn->prepare("INSERT INTO vendor(vid,name,address,pincode,phno,image) VALUES(?,?,?,?,?,?);");
