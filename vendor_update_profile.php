...
if(isset($_POST['update_profile'])){
   ...
   if(!empty($update_pass) AND !empty($new_pass) AND !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         $update_pass_query->execute([$confirm_pass, $vid]);
   ...
   $update_profile->execute([$email, $vid]);
   $update_profile = $conn->prepare("UPDATE `vendor` SET  name = ?, address = ?, pincode = ? , phno =? WHERE vid = ?");
   $update_profile->execute([$name, $address, $pincode ,$phno, $vid]);
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `vendor` SET image = ? WHERE vid = ?");
